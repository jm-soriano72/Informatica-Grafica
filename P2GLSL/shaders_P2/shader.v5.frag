...
//Propiedades Luz 1
vec3 Id = vec3(20.0); //Intensidad de la luz difusa a una distancia d0
vec3 Is = vec3(20.0); //Intensidad de la luz especular a una distancia d0
vec3 lpos = vec3(3.0); //Posición de la luz en coordenadas del mundo, que habrá que convertir a coordenadas de la cámara

//Propiedades Luz 2
vec3 Id2 = vec3(30.0); //Intensidad difusa de la luz 2 a una distancia d0
vec3 Is2 = vec3(30.0); //Intensidad especular de la luz 2 a una distancia d0
...
vec3 shadePuntual () {
	...
	// FUNCIÓN DE ATENUACIÓN 2 //
	float d2 = distance(lpos2, pos); //Distancia entre el foco de luz y la posición del objeto 
	float atenuacion2 = fDist(d0, d2, d_max); //Se calcula el factor de atenuación con la distancia a través de la función auxiliar "fDist"
