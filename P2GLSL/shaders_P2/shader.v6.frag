...
vec3 Id3 = vec3(1.0); //Intensidad difusa de la luz direccional
vec3 Is3 = vec3(1.0); //Intensidad especular de la luz direccional
vec3 dirl3 = vec3(0.2,-1.0,0.0); //Dirección de la luz direccional


//Propiedades Luz 4 -> Luz focal

vec3 Id4 = vec3(2.0); //Intensidad difusa de la luz focal
vec3 Is4 = vec3(2.0); //Intensidad especular de la luz focal
vec3 lpos4 = vec3(0.0,0.0,-4.0); //Posición del foco
...
float df = 1.0; // Mide la densidad de la niebla (profundidad)
vec3 cFog = vec3(0.0); // Color de la niebla 
float posCamara_z = view[3].z; // Posición de la cámara en el eje Z
float df2 = 0.6; //Mide la densidad de la niebla (altura)
